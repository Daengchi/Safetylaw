...
def _data_cell(ws, row: int, col: int, value, bg: str, wrap: bool = False,
               bold: bool = False, valign: str = "top", halign=None) -> None:
...
def _add_law_list_sheet(ws, law_groups: list[dict],
    ...
    for gi, group in enumerate(law_groups):
        ...
        for i, law in enumerate(laws):
            ...
            _data_cell(ws, row, 2, law["name"], bg, bold=is_parent, valign="center")
            _data_cell(ws, row, 3, 개정일,      bg, valign="center", halign="center")
            _data_cell(ws, row, 4, 시행일,      bg, valign="center", halign="center")
            _data_cell(ws, row, 5, cnt_val,     bg, valign="center", halign="center")
            _data_cell(ws, row, 6, 비고,        bg, valign="center", halign="center")
...
# inlineStr 셀(본문 보유): <c ...t="inlineStr"...><is>본문</is></c>
# 본문은 단순 <t>..</t> 또는 rich text <r>..</r>.. — <is>와 <si>는 자식 구조가 같다.
_INLINE_CELL_RE = re.compile(
    r'<c\b([^>]*?)\st="inlineStr"([^>]*?)><is>(.*?)</is></c>', re.DOTALL)
_WS_FILE_RE = re.compile(r'^xl/worksheets/sheet\d+\.xml$')
# t="inlineStr"인데 <is> 본문이 없는 빈 self-closing 셀 (openpyxl 3.1.5가
# 빈 문자열 값에 대해 생성) → t 속성 제거하여 빈 셀로 정규화
_EMPTY_INLINE_RE = re.compile(r'(<c\b[^>]*?)\s+t="inlineStr"(\s*/>)')
# openpyxl이 줄바꿈만 있는 rich text run의 <t>에 xml:space="preserve"를 빠뜨려
# Excel이 "문자열 속성" 손상으로 복구한다. 앞뒤 공백이 있는 plain <t>에 보강한다.
_T_PRESERVE_RE = re.compile(r'<t>([^<]*)</t>')


def _preserve_space(inner: str) -> str:
    def repl(m: re.Match) -> str:
        txt = m.group(1)
        if txt and txt != txt.strip():
            return f'<t xml:space="preserve">{txt}</t>'
        return m.group(0)
    return _T_PRESERVE_RE.sub(repl, inner)


def _fix_shared_strings(xlsx_path: str) -> None:
    """
    openpyxl 3.1.5는 모든 문자열을 inlineStr로 출력하나, 일부 Excel은 이를
    "문자열 속성" 손상으로 보고 복구한다. Excel 자체 포맷처럼 모든 inlineStr
    셀(단순 + rich text)을 sharedStrings 참조(t="s")로 이전해 정규화한다.
    """
    ...
    # ── 1단계: 모든 inlineStr <is> 본문 수집 (본문은 이미 유효한 XML) ───────────
    ...
    for name in names:
        ...
        for m in _INLINE_CELL_RE.finditer(xml):
            inner = m.group(3)
            total_refs += 1
            if inner not in content_to_idx:
                content_to_idx[inner] = len(si_list)
                si_list.append(inner)
    ...
    def _replace(m: re.Match) -> str:
        return f'<c{m.group(1)} t="s"{m.group(2)}><v>{content_to_idx[m.group(3)]}</v></c>'
    ...
    with zipfile.ZipFile(zout_buf, 'w', compression=zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if _WS_FILE_RE.match(item.filename):
                s = data.decode('utf-8')
                s = _INLINE_CELL_RE.sub(_replace, s)
                s = _EMPTY_INLINE_RE.sub(r'\1\2', s)   # 빈 inlineStr 속성 제거
                data = s.encode('utf-8')
        ...
        # ── 3단계: sharedStrings.xml 생성 (본문은 재이스케이프하지 않음) ─────────
        lines = [
            '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
            f'<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"'
            f' count="{total_refs}" uniqueCount="{len(si_list)}">',
        ]
        for inner in si_list:
            lines.append(f'<si>{_preserve_space(inner)}</si>')
        lines.append('</sst>')
